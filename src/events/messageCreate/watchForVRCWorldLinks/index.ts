...
const watchForVRCWorldLinks = async (message: Message): Promise<void> => {
  ...
  try {
    ...
    const forwardingChannels = await getForwardingChannels(worldData, supportedPlatforms);
    
    for (const channel of forwardingChannels) {
      await forwardToChannel(message, channel.id, channel.tag, embed, worldData.id);
    }
